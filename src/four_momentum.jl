...
coordinate_system(::SFourMomentum) = EXYZ()

@inline energy(p::SFourMomentum) = p.E
@inline px(p::SFourMomentum) = p.px
@inline py(p::SFourMomentum) = p.py
@inline pz(p::SFourMomentum) = p.pz
...
function isonshell(mom::QEDbase.AbstractLorentzVector{T}) where {T<:Real}
    rho2 = magnitude2(mom)
    E = energy(mom)
    return isapprox(E^2, rho2; rtol=eps(T))
end
...
function isonshell(mom::QEDbase.AbstractLorentzVector{T}, mass::Real) where {T<:Real}
    ...
    rho2 = magnitude2(mom)
    E = energy(mom)
    return isapprox(E^2, (mass)^2 + rho2; atol=2 * eps(T), rtol=eps(T))
end
